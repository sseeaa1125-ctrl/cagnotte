...
import { copyToClipboard, getStoreUrl } from "@/lib/share";
...
export function ShareSheet({ slug, displayName, onClose }: ShareSheetProps) {
  const [copied, setCopied] = useState(false);
  const [mounted, setMounted] = useState(false);
  const url = getStoreUrl(slug);
  const text = `Découvre ${displayName} sur izy.store`;
  ...
  const sheet = (
    <>
      ...
      {/* Bottom sheet */}
      <div
        className="fixed inset-x-0 bottom-0 z-[61] max-h-[92vh] overflow-y-auto rounded-t-3xl px-6 pb-[max(2rem,calc(env(safe-area-inset-bottom)+1.25rem))] pt-5 shadow-2xl sm:inset-auto sm:left-1/2 sm:top-1/2 sm:max-h-[90vh] sm:w-[380px] sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-3xl sm:pb-8"
        style={{ backgroundColor: "var(--theme-modal-bg, #FFFFFF)", WebkitOverflowScrolling: "touch", touchAction: "pan-y" }}
      >
        {/* Handle bar (mobile) */}
        ...
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold" style={{ color: "var(--theme-modal-text, #111827)" }}>Partager</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center justify-between gap-2">
          {SHARE_TARGETS.map((target) => (
            <button
              ...
              className="flex flex-col items-center gap-1.5 rounded-2xl px-2 py-3 transition-colors active:scale-95 min-w-0 flex-1"
            >
              <div
