...
Plans:
- [ ] 01-01: Prisma schema migration — add `Block.slug` + reserved-words guard, `Order.isAnonymous/messageIsPrivate`, `Notification` model + `dedupeKey @unique`, `WebhookLog @@unique([externalId,eventType])`. Use `prisma migrate dev` (not `db push`) so prod gets a named migration. Verify with `db:studio`.
